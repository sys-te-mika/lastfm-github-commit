...
test('Worker updates KV once and serves a no-cache SVG', async () => {
  ...
  assert.equal(result.listeningStatus.track, 'Live <Song>');
  ...
  assert.match(svg, /Currently playing Artist &amp; Co — Live &lt;Song&gt;/);
  assert.match(svg, /<desc id="desc">[^<]*Currently playing Artist &amp; Co — Live &lt;Song&gt;\.<\/desc>/);
