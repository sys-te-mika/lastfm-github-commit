...
import { dateKey, deduplicate, escapeXml, intensityLevel, intensityThresholds, renderSvg, THEMES } from '../src/lib.js';
...
test('SVG generation is deterministic and escapes external text', () => {
  const options = {
    ...
    listeningStatus: { kind: 'now-playing', artist: 'A&B', track: '<Song>' },
  ...
  assert.match(first, /Now playing at last update: A&amp;B — &lt;Song&gt;/);
